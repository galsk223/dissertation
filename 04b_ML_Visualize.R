longout <- read_rds("~/westcoast-networks/data/clean/Simulation/projectiondataML_longout.rds")

tpde <- expand_grid(Type = 1:2,
                    Projection = 1:3,
                    Data = 1:4,
                    Engine = 1:3)
...
sum_long <- map_dfr(1:nrow(tpde), function(j){
  ...
  temp <- longout[[tpde$Type[[j]]]][[tpde$Projection[[j]]]][[tpde$Data[[j]]]][[tpde$Engine[[j]]]]$sum
...
classification_comp <- sum_long %>% 
  filter(Type != "Regression") %>% 
  mutate(Engine = case_when(Engine == "NNET" ~ "Neural Net",
                            Engine == "GLMNET" ~ "Lasso",
                            Engine == "RANGER" ~ "Random Forest"))

classification_comp$DataSubset <- factor(classification_comp$DataSubset, 
                                     levels = c("NoNetwork", "All", "RegionNetwork", "FisheryNode"))

ggplot(classification_comp, aes(x = Engine, y = ModelPerformance)) +
  geom_point() +
  facet_grid(rows = vars(Projection),
             cols = vars(DataSubset)) +
  theme_minimal() +
  ...
  labs(title = "Classification Model Comparisons",
       subtitle = "Performance in Accuracy")

j <- 38
vi_long <- map_dfr(1:nrow(tpde), function(j){
  
  temp_info <- longout[[tpde$Type[[j]]]][[tpde$Projection[[j]]]][[tpde$Data[[j]]]][[tpde$Engine[[j]]]]$sum
  if(j <= 36){
  temp <- longout[[tpde$Type[[j]]]][[tpde$Projection[[j]]]][[tpde$Data[[j]]]][[tpde$Engine[[j]]]]$vi %>% 
    ...
    temp_pre <- longout[[tpde$Type[[j]]]][[tpde$Projection[[j]]]][[tpde$Data[[j]]]][[tpde$Engine[[j]]]]$vi
