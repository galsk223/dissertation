...
region <- regions[3]
year <- 2017
source("westcoast-networks/scripts/02_Simulation/Port Regions/01_ChoiceFunction_ASC.R", echo = F, verbose = F)
source("westcoast-networks/scripts/02_Simulation/Port Regions/01a_ASC_Calibrate_Functions.R", echo = F, verbose = F)
    ...
    guess <- df_exprev %>% 
      ...
      for (try in 1:10){
       print(try)
        ...
        set.seed(Sys.time())
        asc_sc_try <- runif(length(asc_sc_blanks$asc_sc_0), 25, mean(df_exprev$ExpRev))
        print(e)
        print(asc_sc_0)
      }
      obj_fcn1_f(asc_sc_0)
        ...
        for (i in 1:4){
          ...
          for (j in 1:length(asc_fc_0)) {
            ...
            for (iter in 1:50) {
              ...
              # } else {
                delta_new <- delta_costs - log(pmax(S_obs, 1e-8) / pmax(S_sim, 1e-8)) 
              # }
              ...
              if (max(abs(S_sim - S_obs)) < stol 
                  || max(abs(delta_new - delta_costs)) < dtol
              ) {
